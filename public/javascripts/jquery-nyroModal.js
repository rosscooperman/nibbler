...
jQuery(function($) {
	...
	function processModal(settings) {
		...
		debug('current type is: ' + currentSettings.type);

		if (currentSettings.type != 'rjs') {
		...
		if (from) {
			if (currentSettings.type == 'form') {
				...
				modal.dataReady = true;
				showModal();
			} else if (currentSettings.type == 'rjs') {
				debug('RJS Load: '+currentSettings.url);
				setCurrentSettings({type: 'rjs'});
				var data = {};
				if (currentSettings.selector) {
		...
		if (!resizing) {
			...
			if (currentSettings.type == 'image' || currentSettings.type == 'gallery') {
				...
				if (title != currentSettings.defaultImgAlt) {
					var divTitle = $('<div class=\'label\'>'+title+'</div>');
	...
	function ajaxLoaded(data) {
		...
		modal.tmp.html(currentSettings.selector
			...
			:filterScripts(data));
		debug('AjaxLoaded: type is: ' + currentSettings.type);
		if (currentSettings.type == 'rjs') {
			debug('AjaxLoaded Evaling Response');
			eval(data);
		} else if (modal.tmp.html()) {
			modal.dataReady = true;
			showContentOrLoading();
		} else {
			loadingError();
